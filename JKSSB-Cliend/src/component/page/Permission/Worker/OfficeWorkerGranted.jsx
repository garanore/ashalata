  ...
  const filteredWorkers = userBranches.includes("AllBranch")
    ...
    : PendingWorkers.filter((worker) => {
        const isBranchMatch = userBranches.some((branch) =>
          worker?.WorkerBranchAdd?.includes(branch)
        );
        ...
        if (isManager) {
          ...
          return (
            isBranchMatch && specialDesignations.includes(worker.designation)
          );
        }
              ...
              <tbody>
                {currentWorkers.map((worker) => (
                  <tr key={worker._id}>
                    ...
                    <td>{loggedWorkers[worker.submittedBy]?.accountName}</td>
