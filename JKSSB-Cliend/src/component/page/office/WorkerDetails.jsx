...
const WorkerDetails = () => {
  ...
  const fetchUsers = ({ branch, designation }) => {
    ...
    if (branch && designation) {
      apiUrl = `http://localhost:5000/get-worker-branch-designation?branch=${encodeURIComponent(
        branch
      )}&designation=${encodeURIComponent(designation)}`;
    } else if (branch) {
      apiUrl = `http://localhost:5000/worker-callback-by-branch/${encodeURIComponent(
        branch
      )}`;
    } else if (designation) {
      apiUrl = `http://localhost:5000/get-worker-designation/${encodeURIComponent(
        designation
      )}`;
    }
  ...
  const handleDeleteClick = (worker) => {
    if (window.confirm("Delete the selected Center")) {
      ...
      axios
        .put(
          `http://localhost:5000/workeradmission/ActiveStatus/${worker._id}`,
          {
    ...
    <div className="bg-light container-fluid p-4">
      ...
      <div className="row mb-4">
        <div className="col">
          ...
          >
            ...
            >
              <i className="fas fa-users"></i> কর্মী তালিকা
            </h2>
          </div>
        </div>
      </div>
      ...
      <div className="row mb-4">
        ...
        <div className="col-md-3 mb-3">
          ...
          >
            <i className="fas fa-users"></i> Total Worker
          </label>
          <div className="input-group shadow-sm">
            <span
              className="input-group-text bg-primary text-white"
      ...
      <div className="table-responsive">
        {currentUsers.length > 0 && (
          <table className="table table-hover">
            ...
            <tbody>
              {currentUsers.map((Worker) => (
                <tr key={Worker.workerID}>
                  <td>{Worker.workerID}</td>
                  <td>{Worker.WorkerName}</td>
                  <td>{Worker.WorkerParent}</td>
                  <td>{Worker.WorkerDic}</td>
                  <td>{Worker.WorkerSubDic}</td>
        ...
        {users.length === 0 &&
          ...
          selectedDesignation === "" && (
            <div className="alert alert-info text-center" role="alert">
              ...
              কর্মী তালিকা দেখার জন্য শাখা বা পদবী অথবা শাখা ও পদবী নির্বাচন
              করুণ ।
            </div>
          )}
        ...
        {users.length === 0 &&
          (selectedBranch !== "" || selectedDesignation !== "") && (
            <div className="alert alert-warning text-center" role="alert">
              ...
              নির্বাচিত শাখা বা পদবী অনুযায়ী কোন কর্মী নেই ।
            </div>
          )}
      </div>
      {selectedBranch && (
