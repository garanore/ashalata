...
const ApproveLoan = () => {
    ...
    const fetchPendingSavins = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/openloan/approved"
        );
  ...
  const approveLoans = async (id) => {
    try {
      const response = await axios.post(
        `http://localhost:5000/openloan/approve/${id}`,
  ...
  const cancelLoans = async (id) => {
    try {
      ...
      console.error("Error canceling loan:", error.message);
  ...
  const reviewLoans = async (id) => {
    try {
      ...
      // Remove the loan from the list after it's sent for review
      setPendingLoans(pendingLoans.filter((loan) => loan._id !== id));
      alert("loan sent back for correction");
    } catch (error) {
      console.error("Error reviewing loan:", error.message);
      alert(`Error: ${error.message}`);
    }
  };

  const ViewLoans = (Loans) => {
    navigate("/home/LoansAbout", {
      ...
      <div className="bg-light container-fluid">
        <div className="p-2">
          <div className="border-bottom mb-5">
            <h2 className="text-center mb-4 pt-3">Approve openloan</h2>
          </div>
        </div>
        <div className="p-3">
