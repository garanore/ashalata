...
const BalanceSheet = require("../models/balancesheet.model.js");
const { ObjectId } = require("mongoose").Types;

router.post("/save-balance-sheet", async (req, res) => {
  try {
    const payload = req.body;

    for (const item of payload) {
      // Check if a document already exists for the branch and productCode
      const existingEntry = await BalanceSheet.findOne({
        branch: item.branch,
      ...
      if (existingEntry) {
        ...
        const newEntry = new BalanceSheet(item);
        await newEntry.save();
      }
    }
...
router.get("/get-balance-sheet/:branch/:month", async (req, res) => {
  try {
    ...
    const entries = await BalanceSheet.find({ branch });
